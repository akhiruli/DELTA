...
public class SimulationManager extends SimulationManagerAbstract {
	...
	private void sendFromOrchToDestination(Task task) {
		...
		if (!task.getEdgeDevice().isApplicationPlaced()) {
			...
			if (task.getOffloadingDestination() == ComputingNode.NULL) {
				...
				//Even for failure we are sending the result back
	...
	private void sendTaskToOrchestrator(Task task) {
		...
		//System.out.println("sendTaskToOrchestrator-after- Task: "+task.getId() + " app_id: " + task.getApplicationID() + " : " + task.getComputingNode().getType());
	...
	public boolean taskFailed(Task task, int phase) {
		...
		if (phase == 3 && task.getActualNetworkTime() + task.getWatingTime() + task.getActualCpuTime() > task
				.getMaxLatency()) {
	 ...
	 * @param phase
	 */
	private boolean setFailed(Task task, int phase) {
		failedTasksCount++;
		tasksCount++;
